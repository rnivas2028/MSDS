...
def exercise09():
   ...
   # ------ Place code below here \/ \/ \/ ------
    ...
    response = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json?start=2017-09-01&end=2018-10-05', headers=header)
    res_j = response.json()
...
def exercise11(n):
    ...
    print('array_reshaped:', array_reshaped)
...
def exercise12(n):
    ...
    print('Checkerboard Matrix :', checkerboard_matrix)
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    nums = np.random.default_rng().integers(low = 0, high = n, size = n, endpoint = True)
