...
class PandasChain:
    ...
    def __commit_block(self, block):
        ...
        block_hash = block.set_block_hash(hashlib.sha256(str(str(self.__prev_hash) + str(self.__id) + str(dt.datetime.now()) + str(self.__seq_id) + str(np.random.randint(low = 1, high = 99)) + str(merkle_root)).encode('utf-8')).hexdigest())
        self.__prev_hash = block_hash#block.get_block_hash()
    ...
    def display_block_headers(self):
        ...
        #return or print self.__seq_id,self.__status,self.__block_hash,self.__prev_hash,self.__merkle_tx_hash,block.get_size()
...
#            [value_list.append(i) for i in block_vals]
#        return value_list
        curr_vals = self.__current_block.get_values()
        value_list = [x for x in curr_vals]
        for block in self.__chain:
            block_vals = block.get_values()
            [value_list.append(x) for x in block_vals]
            #value_list.append(committed_list)
        return value_list
...
class Block:
    ...
    def display_header(self):
        return self.__seq_id, self.__status, self.__block_hash,  self.__prev_hash, self.__merkle_tx_hash, self.__transactions
    ...
    def add_transaction(self, s, r, v):
        ...
        #self.__transactions = pd.DataFrame([[ts, s, r, v, tx_hash]], columns=self.__col_names)  #Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def set_status(self, status):
        if status == "COMMITTED":
            self.__status = "COMMITTED" #["COMMITTED","UNCOMMITTED"]

    # 5 pts - Setter for block hash
    #- Block hash: previous block's hash, the chains hash id, current date time, sequence id of the block, a random integer between 0 and 99 and the root Merkle hash.
    def set_block_hash(self, hash):
        self.__block_hash = hash
#        pandas_chain = PandasChain('testnet')
#        temp_hash = hashlib.sha256(str(str(hash) + str(self.__prev_hash)+str(pandas_chain.self.__id)+str(dt.datetime.now())+str(self.__seq_id)+str(np.random.randint(0,99))+ str(self.__merkle_tx_hash)).encode('utf-8')).hexdigest()
#        self.__block_hash = temp_hash
#        return self.__block_hash
    ...
    def get_simple_merkle_root(self):
        my_list = list(self.__transactions['TxHash'])
        my_list_str = ''.join(map(str,my_list))
        self.__merkle_tx_hash = hashlib.sha256(my_list_str.encode('utf-8')).hexdigest()
    ...
    def get_values(self):
        #print("I am in get values section")
        return self.__transactions['Value']
        #return sum(self.__transactions['Value'])
