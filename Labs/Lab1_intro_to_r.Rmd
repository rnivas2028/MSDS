...
## Adding total to the data frame
...
```{r plot-girls-vs-year-line-trend}
ggplot(arbuthnot, aes(x=year,y=girls))+geom_point()+
  geom_line()+stat_smooth(fill="grey")+theme_bw()
...
```{r calc-prop-boys-born-over-time}
...
1.  Make a plot that displays the proportion of boys born over time. What do you see? 
    ...
    Boy-to-girl ratio for every year with plot
```{r}
present$boys/present$girls
plot(x = present$year, y = present$boys /present$girls, type = "l", col = "black", lwd="1.5", main = "Present Boy-To-Girl Ratio", xlab = "Year", ylab = "Boy-to-Girl Ratio")
```

Boy to Girl proportion over the years
```{r}
present$boys / (present$boys + present$girls)
plot(x = present$year, y = present$boys /(present$boys +present$girls), type = "l", col = "black", lwd="1.5", main = "Present Proportion of Boys", xlab = "Year", ylab = "Proportion of Boys")
```
Based on the analysis above, Boy-to-girl ratio as well as proportion are decreasing over the time.Comparison shows that the Arbuthnot’s observation about boys being born in greater proportion than girls holds up.
...
legend(1990, 1800000, c("Boys","Girls"), lty=c(1,1), lwd=c(1.5,1.5), col=c("blue","red"))
