...
```{r}
# Visualize the distributions of the amount of calories from fat
hist(mcdonalds$cal_fat,
main="Mcdonalds : Amount of calories from fat",
...
main="Dairy Queen : Amount of calories from fat",
xlab="Calories from Fat",
ylab="Frequency",
col="cornsilk"
)
...
```
McDonald food has a higher minimum, maximum and mean / center for fat calories, and the McDonald’s x axis increases in increments of 200 cals while the Dairy Queen one increases in increments of 100 cals. Both plots are right skew. McDonalds data has a bigger spread (1200-1400), while the dairy queen maximum is between 600-700.
...
Answer:
Yes, the data follows a normal distribution. I looks like the center is slightly left of the the mode. 
...
```{r}
set.seed(2);
hist(sim_norm, probability = TRUE, xlab="Simulated Normal Distribution",)
x <- -200:800
y <- dnorm(x = x, mean = mean(sim_norm), sd = sd(sim_norm))
lines(x = x, y = y, col = "blue")

qqnorm(sim_norm, main='Q-Q Plot for Female heights')
qqline(sim_norm)
...
Answer:
Yes, the normal QQ plot for 'fdim$hgt' looks similar to the simulated plots and so provides evidence that the data are nearly normal.
...
Answer:
The points form more of a curve instead of a straight line. Normal Q-Q plots that look like the weight data is skewed.
```{r}
...
Answer:
The two probability question are as below; Based on this the variable height had a closer agreement between the two methods.
1. Percent of females have a height of 152.4 cm (5 feet) or less
...
2. Percent of females have a weight of 80kg or more?
```{r}
pnorm(q = 80, mean = fwgtmean, sd = fwgtsd)
...
ggplot(data = mcdonalds, aes(x = sodium)) +
        ...
        stat_function(fun = dnorm, args = c(mean = mc_mean, sd = mc_sd ), col = "blue")

dq_mean <- mean(dairy_queen$sodium)
dq_sd   <- sd(dairy_queen$sodium)
ggplot(data = dairy_queen, aes(x = sodium)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..), bins=20) +
        stat_function(fun = dnorm, args = c(mean = dq_mean, sd = dq_sd), col = "red")

```
McDonalds distribution is the closest to normal for sodium.
...
Answer:
This might be a case because the sodium data is a finite scale and the numbers are whole (no decimal) that would be explanation for a stepwise pattern.
...
```{r}
c_mean <- mean(dairy_queen$total_carb)
c_sd   <- sd(dairy_queen$total_carb)
sim_norm5 <- rnorm(n = nrow(dairy_queen), mean = c_mean, sd = c_sd)
...
ggplot(data = dairy_queen, aes(x = total_carb)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..), bins=20) +
        stat_function(fun = dnorm, args = c(mean = c_mean, sd = c_sd), col = "red")
```
\
Distribution is normal with right skewed\
* * *